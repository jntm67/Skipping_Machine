...
const float speed_factor = 60.0f / (float)counts_per_revolution / 0.05f;
...
int main(void) {
	...
	// Start input capture interrupt for TIM2 channels 2 and 3
	HAL_TIM_IC_Start_IT(&htim2, TIM_CHANNEL_2);
	HAL_TIM_IC_Start_IT(&htim2, TIM_CHANNEL_3);

	motor.target = 60.0f;
	motor.pid_mode = POSITION_PID;
	/* USER CODE END 2 */
...
void HAL_TIM_ElapsedCallback(TIM_HandleTypeDef *htim) {
	if (htim == &htim4) {
		...
		cnt_prev = cnt_now;
		float speed_rpm = delta * speed_factor;
		pid_cal(&motor);
		if (motor.out > max_v) {
			motor.out = max_v;
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim) {
	if (htim->Instance == TIM2) {
		// Note: HAL stores the active channel in htim->Channel
		if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_2) {
			// On rising edge of encoder A: increment motor.target
			motor.target++;
		} else if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_3) {
			// On rising edge of encoder B: decrement motor.target
			motor.target--;
		}
	}
}
